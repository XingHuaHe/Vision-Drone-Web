...
def route_conv(layer_index : int, module_defs : Dict) -> int:
    """ find the convolutional layers connected by route layer"""
    module_def = module_defs[layer_index]
    mtype = module_def['type']
    
    before_conv_id = []
    if mtype in ['convolutional', 'shortcut', 'upsample', 'maxpool', 'spk']:
...
def write_model_cfg(old_path : str, new_path : str, new_module_defs : Dict) -> None:
    ...
    for i, module_def in enumerate(new_module_defs):
        ...
        if mtype == "convolutional":
            ...
            lines.append("from={}\n".format(module_def['from'][0]))
            lines.append("activation={}\n\n".format(module_def['activation']))   
        elif mtype == 'route':
            lines.append("layers={}\n\n".format(', '.join(str(i) for i in module_def['layers'])))
        elif mtype == 'spk':
            lines.append("out_features={}\n".format(module_def['out_features']))
            lines.append("WH={}\n".format(module_def['WH']))
            lines.append("M={}\n".format(module_def['M']))
            lines.append("G={}\n".format(module_def['G']))
            lines.append("r={}\n\n".format(module_def['r']))
            ...
            lines.append("mask = {}\n".format(', '.join(str(i) for i in module_def['mask'])))
            lines.append("anchors = {}\n".format(', '.join(str(int(i)) for i in module_def['anchors'].reshape(-1))))
            ...
            lines.append("scale_x_y = {}\n".format(module_def['scale_x_y']))
            lines.append("iou_thresh = {}\n".format(module_def['iou_thresh']))
            lines.append("cls_normalizer = {}\n".format(module_def['cls_normalizer']))
            lines.append("iou_normalizer = {}\n".format(module_def['iou_normalizer']))
            lines.append("iou_loss = {}\n".format(module_def['iou_loss']))
            # lines.append("random = {}\n".format(module_def['random']))
            lines.append("nms_kind = {}\n".format(module_def['nms_kind']))
            lines.append("beta_nms = {}\n\n".format(module_def['beta_nms']))
...
def test(
        ...
        img_size : int = 608,
        save : str = None,
        overall_ratio : int = 0.5,
        perlayer_ratio : int = 0.1
) -> None:

    """prune yolov4-spk-spp and generate pruned cfg and weights"""
    if save != None:
    ...
    for m in model.modules():
        module_set.add(m.__class__.__name__)
    ...
    for i, (module_def, module) in enumerate(zip(model.module_defs, model.module_list)):
        print("layer:", i)
        mtype = module_def['type']
        if mtype  == 'convolutional' and model.module_list[i+1].__class__.__name__ != 'SPKUnit':
            bn = int(module_def['batch_normalize'])
            if bn:
            ...
            print("layer:", mtype)
        elif mtype  == 'convolutional' and model.module_list[i+1].__class__.__name__ == 'SPKUnit':
            bn = int(module_def['batch_normalize'])
            if bn:
                m = getattr(module, 'BatchNorm2d')
                weight_copy = m.weight.data.abs().clone()
                mask = torch.ones(m.weight.data.abs().shape)
                proned_module_defs[i]['mask'] = mask.clone()
            ...
            layer_i = int(module_def['from'][0])+i
    ...
    for i in range(layer_number-1, -1, -1):
        ...
        if mtype == 'shortcut': 
            ...
            while mtype == 'shortcut':
                ...
                layer_i = int(proned_module_defs[layer_i]['from'][0])+layer_i 
    ...
    ckpt['model'] = pruned_state_dict
    torch.save(ckpt, prune_weights_path)
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(description='PyTorch Slimming Yolov4-SPK-SPP prune')
    parser.add_argument('--cfg', type=str, default='./cfg/yolov4-spk-spp.cfg', help='cfg file path')
    parser.add_argument('--weights', type=str, default='./weights/last_295_spk_spp.pt', help='path to weights file')
    parser.add_argument('--img_size', type=int, default=608, help='inference size (pixels)')
    parser.add_argument('--save', default='./prunes', type=str, metavar='PATH', help='path to save pruned model (default: none)')
    parser.add_argument('--overall_ratio', type=float, default=0.5, help='scale sparse rate (default: 0.5)')    
    parser.add_argument('--perlayer_ratio', type=float, default=0.5, help='minimal scale sparse rate (default: 0.1) to prevent disconnect')    
    
    opt = parser.parse_args()
    opt.save += "/prune_{}_{}".format(opt.overall_ratio, opt.perlayer_ratio)
