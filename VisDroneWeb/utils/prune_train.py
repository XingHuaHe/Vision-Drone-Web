...
def train(hyp, opt, device, tb_writer=None):
    ...
    last = wdir + 'last_prune.pt'
    best = wdir + 'best_prune.pt'
    ...
    if opt.adam:
        ...
        optimizer = optim.SGD(pg0, lr=ckpt['optimizer']['param_groups'][0]['lr'], momentum=hyp['momentum'], nesterov=True)
    ...
    # Model parameters
    hyp['cls'] *= nc / 10.  # scale coco-tuned hyp['cls'] to current dataset
    ...
    for epoch in range(start_epoch, epochs):  # epoch ------------------------------------------------------------------
        ...
        if rank in [-1, 0]:
            ...
            if not opt.notest and final_epoch:  # Calculate mAP
            ...
            if save:
                ...
                if epoch % 5 == 0:
                    torch.save(ckpt, last.replace('.pt','_{:04d}.pt'.format(epoch)))
                # if (best_fitness == fi) and not final_epoch:
                #     torch.save(ckpt, best)
                del ckpt

            # torch.save(model.state_dict(), f'./output/checkpoint_{epoch}.pt')
        # end epoch ----------------------------------------------------------------------------------------------------
    # end training
...
if __name__ == '__main__':
    ...
    parser.add_argument('--weights', nargs='+', type=str, default='./prunes/prune_0.5_0.5/prune.pt', help='model.pt path(s)')
    # parser.add_argument('--cfg', type=str, default='./cfg/yolov4-tiny.cfg', help='model .cfg file path')
    parser.add_argument('--cfg', type=str, default='./prunes/prune_0.5_0.5/prune.cfg', help='*.cfg path')
    ...
    parser.add_argument('--notest', type=bool, default=True, help='only test final epoch')
    parser.add_argument('--noautoanchor', action='store_true', help='disable autoanchor check')
    parser.add_argument('--evolve', action='store_true', help='evolve hyperparameters')
    parser.add_argument('--bucket', type=str, default='', help='gsutil bucket')
    parser.add_argument('--cache-images', action='store_true', help='cache images for faster training')
    parser.add_argument('--name', default='yolov4', help='renames results.txt to results_name.txt if supplied')
    ...
    # Resume.(when weights is None or '' and opt.resume == 'get_last', avaliable.)
    if opt.resume:
        last = get_latest_run(search_dir='./runs') if opt.resume == 'get_last' else opt.resume  # resume from most recent run
    ...
    # Check weather the files is exist.
    ...
    if not opt.evolve:
        ...
        if opt.global_rank in [-1, 0]:
            print('Start Tensorboard with "tensorboard --logdir=./%s", view at http://localhost:6006/' % opt.logdir[0:-1])
