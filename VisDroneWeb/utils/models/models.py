...
def create_modules(module_defs, img_size, cfg):
    ...
    for i, mdef in enumerate(module_defs):
        ...
        # selectived kernel and channel spatial attention machanism
        elif mdef['type'] == 'skcs':
            in_features = output_filters[-1]
            filters = mdef['out_features']
            WH = mdef['WH']
            M = mdef['M']
            G = mdef['G']
            r = mdef['r']
            modules = SKCSUnit(in_features, filters, WH, M, G, r)

        # selectived channel kernel.
        elif mdef['type'] == 'sck' or mdef['type'] == 'ssk' or mdef['type'] == 'spk':
            in_features = output_filters[-1]
            filters = mdef['out_features']
            WH = mdef['WH']
            M = mdef['M']
            ...
            r = mdef['r']
            if mdef['type'] == 'sck':
                modules = SCKUnit(in_features, filters, WH, M, G, r)
            # selectived spatial kernel.
            elif mdef['type'] == 'ssk':
                modules = SSKUnit(in_features, filters, WH, M, G, r)
            # 
            elif mdef['type'] == 'spk':
                modules = SPKUnit(in_features, WH, M, G, r)
